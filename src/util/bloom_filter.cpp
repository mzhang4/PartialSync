...
int main() {
  ...
  opt.false_positive_probability = 0.01;
  opt.projected_element_count = 10000;
  opt.compute_optimal_parameters();
  bloom_filter bf(opt);

  for (int i = 0; i < 10000; i++) {
    bf.insert("/a/b/"+to_string(i));
  }

  for (int i = 0; i < 10000; i++) {
    if(!bf.contains("/a/b/"+to_string(i)))
      cout << i << endl;
  }
  ...
  cout << cnt << " " << cnt*1./(200000-10000) << endl;
}*/